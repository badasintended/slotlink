...
class SlotCountWidget(
...
) : AbstractButtonWidget(x, y, backgroundWidth, getClient().textRenderer.fontHeight, LiteralText.EMPTY) {

    private var textX = x

    override fun render(matrices: MatrixStack, mouseX: Int, mouseY: Int, delta: Float) {
        val text = (total.invoke() - filled.invoke()).toFormattedString()
        getClient().textRenderer.apply {
    ...
    override fun renderToolTip(matrices: MatrixStack, mouseX: Int, mouseY: Int) {
        if ((textX < mouseX) and (mouseX < x + width - 7)) getClient().currentScreen?.apply {
