...
class FilterSlotWidget(
    ...
    override fun appendTooltip(tooltip: MutableList<Text>) {
        tooltip.add(TranslatableText("container.slotlink.filter.slot.nbt.${nbt}").formatted(Formatting.GRAY))
        tooltip.add(TranslatableText("container.slotlink.filter.slot.nbt.scroll").formatted(Formatting.GRAY))
    }
